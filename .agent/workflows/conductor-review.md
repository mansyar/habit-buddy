...
2.  **Auto-Detect Scope:**
    ...
    - If one exists, present the following confirmation to the user in the chat and await their response:
      > "Do you want to review the in-progress track '<track_name>'? (yes/no)"
    - If no track is in progress, or user says "no", ask the user in the chat and await their response:
      > **Select Scope**
      > What would you like to review?
      > _(e.g., Enter track name, or 'current' for uncommitted changes)_
3.  **Confirm Scope:** Ensure you and the user agree on what is being reviewed by presenting in the chat and awaiting their response:
    > "I will review: '<identified_scope>'. Is this correct? (yes/no)"
...
3.  **Load and Analyze Changes (Smart Chunking):**
    ...
    - **Strategy Selection:**
      ...
      - **Large Changes (> 300 lines):**
        - **Confirm:** Present the following to the user in the chat and await their response:
          > "This review involves >300 lines of changes. I will use 'Iterative Review Mode' which may take longer. Proceed? (yes/no)"
...
4.  **Testing:**
    ...
    - _Action:_ **Execute the test suite automatically.** Infer the test command based on the codebase languages and structure (e.g., `npm test`, `pytest`, `go test`). Run it. Analyze the output for failures.
...
_(Only include this section if issues are found)_
...
2.  **Action:**
    - **If issues found:** Present the following choices to the user in the chat and await their response:
      > **Decision**
      > How would you like to proceed with the findings?
      >
      > 1. **Apply Fixes** - Automatically apply the suggested code changes.
...
2.  **Condition for Action:**
    ...
    - If changes are detected:
      a. **Check for Track Context:** - If you are NOT reviewing a specific track (i.e., you don't have a `plan.md` in context), present the following to the user in the chat and await their response: > "I've detected uncommitted changes. Should I commit them? (yes/no)" - If 'yes', stage all changes and commit with `fix(conductor): Apply review suggestions <brief description of changes>`. - Proceed to '3.3 Track Cleanup'.
...
2.  **Ask for User Choice:** Present the following choices to the user in the chat and await their response:

    > **Track Cleanup**
    > Review complete. What would you like to do with track '<track_name>'?
    >
    > 1. **Archive** - Move the track's folder to `conductor/archive/` and remove it from the tracks file.
    > 2. **Delete** - Permanently delete the track's folder and remove it from the tracks file.
    > 3. **Skip** - Do nothing and leave it in the tracks file.
...
3.  **Handle User Response:**
    ...
    - **If "Delete":**
      i. **Confirm:** Present the following warning to the user in the chat and await their confirmation: > ⚠️ WARNING: This is an irreversible deletion. Do you want to proceed? (yes/no)
